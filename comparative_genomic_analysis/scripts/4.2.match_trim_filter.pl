...
unless(-e "$pos_selection/4.2.trimal_filter_cds_match.ok"){
	...
	foreach my $filename ( @array ){
		...
		if($len_uniq == 1){
			...
			print PA_TRI "trimal -in $pos_selection/$ps_dir3_1/$filename -out $pos_selection/$ps_dir3_2/$trim_out -nogaps\n";
		}
	}close PA_TRI;
	system("ParaFly -c $pos_selection/para_trim.txt -CPU $cpu");
